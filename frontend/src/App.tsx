...
export function App() {
  ...
  const poolEmpty = !reserveValues || (reserveValues[0] === 0n && reserveValues[1] === 0n);
  ...
  const token1Balance = useContractRead(token1Address, erc20Abi, 'balanceOf', [address],
  ...
  async function runTx(action: () => Promise<`0x${string}`>) {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('user rejected'))    setUiError('Transaction rejected in wallet.');
      else if (msg.toLowerCase().includes('insufficient')) setUiError('Insufficient balance or allowance. Did you approve first?');
      else if (msg.toLowerCase().includes('liquidity'))    setUiError('Pool has no liquidity. Add liquidity before swapping.');
      else if (msg.toLowerCase().includes('invalid token')) setUiError('Pool token mismatch. Check pool configuration.');
      else setUiError(msg.slice(0, 200));
    }
  }

  function approve(spender: Address, amount: bigint, token: Address) {
    return writeContractAsync({ address: token, abi: erc20Abi, functionName: 'approve', args: [spender, amount], gas: 100_000n });
  }
    ...
    <main className="page">
      ...
      <section className="hero">
        <div>
          ...
          <p>AMM · ERC-4626 vault · DAO governance · Chainlink oracle · The Graph — Base Sepolia</p>
        </div>
      ...
      {!isConfigured(tokenAddress) && (
        <div className="warning">Update src/addresses.ts with deployed contract addresses.</div>
      )}
      ...
      <section className="grid">
        ...
        <article className="card">
          ...
          <button
            onClick={() => runTx(() => writeContractAsync({
              address: tokenAddress, abi: governanceTokenAbi,
              functionName: 'delegate', args: [address!], gas: 150_000n,
            }))}
          >
            Delegate to myself
          </button>
        ...
        <article className="card">
          <h2>AMM Pool</h2>
          <div className="row"><span>Token0</span><b className="addressText">{token0Address}</b></div>
          <div className="row"><span>Token1</span><b className="addressText">{token1Address}</b></div>
          ...
          {isConnected && (
            <div className="row"><span>Your token1 balance</span><b>{fmt(token1Balance.data as bigint)}</b></div>
          )}

          {poolEmpty && (
            <div className="warning" style={{fontSize:'12px',padding:'10px 14px'}}>
              Pool has no liquidity — add liquidity first before swapping.
            </div>
          )}

          {/* Add Liquidity */}
          <p style={{fontSize:'11px',color:'var(--txt-2)',marginTop:'4px'}}>① Add liquidity (approve both tokens first)</p>
          <input value={liqAmount0} onChange={e => setLiqAmount0(e.target.value)} placeholder="Token0 amount" />
          <input value={liqAmount1} onChange={e => setLiqAmount1(e.target.value)} placeholder="Token1 amount" />
          <button onClick={() => runTx(() => approve(ammAddress, parseEther(liqAmount0), token0Address))}>
            Approve token0
          </button>
          <button onClick={() => runTx(() => approve(ammAddress, parseEther(liqAmount1), token1Address))}>
            Approve token1
          </button>
          <button onClick={() => runTx(() => writeContractAsync({
            address: ammAddress, abi: ammAbi, functionName: 'addLiquidity',
            args: [parseEther(liqAmount0), parseEther(liqAmount1), 0n, 0n, address!],
            gas: 400_000n,
          }))}>
            Add Liquidity
          </button>
          ...
          {/* Swap */}
          <p style={{fontSize:'11px',color:'var(--txt-2)',margin:'0'}}>② Swap (approve token0, then swap)</p>
          <input value={amountIn} onChange={e => setAmountIn(e.target.value)} placeholder="Token0 amount to swap" />
          <button onClick={() => runTx(() => approve(ammAddress, parseEther(amountIn), token0Address))}>
            Approve token0
          </button>
          <button
            disabled={poolEmpty}
            ...
            onClick={() => runTx(() => writeContractAsync({
              address: ammAddress, abi: ammAbi, functionName: 'swap',
        ...
        <article className="card">
          ...
          <div className="row"><span>Vault asset</span><b className="addressText">{assetAddress}</b></div>
          ...
          <input value={depositAmount} onChange={e => setDepositAmount(e.target.value)} placeholder="Amount to deposit" />
          <button onClick={() => runTx(() => approve(vaultAddress, parseEther(depositAmount), assetAddress))}>
            Approve vault asset
          </button>
          <button onClick={() => runTx(() => writeContractAsync({
        ...
        <article className="card">
          ...
          <input
            ...
            style={proposalId && !proposalIdValid ? {borderColor:'var(--red)'} : {}}
          />
          {proposalId && !proposalIdValid && (
            <p style={{fontSize:'11px',color:'var(--red)',margin:'0'}}>Invalid proposal ID — must be a number</p>
          )}
          <button
            disabled={!proposalIdValid}
            onClick={() => {
              if (!proposalIdBig) return setUiError('Enter a valid proposal ID.');
              runTx(() => writeContractAsync({
                address: governorAddress, abi: governorAbi,
                functionName: 'castVote', args: [proposalIdBig, 1], gas: 150_000n,
              }));
            }}
          >
            Vote For
          ...
          <button
            ...
            onClick={() => {
              ...
              runTx(() => writeContractAsync({
                address: governorAddress, abi: governorAbi,
                functionName: 'castVote', args: [proposalIdBig, 0], gas: 150_000n,
              }));
            }}
          >
            Vote Against
      ...
      <section className="card full">
        ...
        {proposals.length === 0 ? (
          <p>No indexed proposals yet. Deploy the subgraph and set subgraphUrl in addresses.ts.</p>
        ) : (
          <div className="table">
            {proposals.map(p => (
